...
export const icons = (pack: string, name: string) => {
    const iconPack = iconPacks[pack];

    if (iconPack && iconPack[name]) {

        if (iconPack[name].path) {
            return `<img src="${iconPack[name].path}" />`;
        }

        if (iconPack[name].base64) {
            return `<img data="${iconPack[name].base64}" />`;
        }

        if (iconPack[name].svg) {
            return `<svg><path d="${iconPack[name].svg}" /></svg>`;
        }
    }
