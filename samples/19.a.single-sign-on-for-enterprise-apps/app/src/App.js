...
const App = () => {
  ...
  const handleGitHubAccessTokenChange = useCallback(
    accessToken => {
      setGitHubAccessToken(accessToken);
      accessToken
        ? sessionStorage.setItem(GITHUB_OAUTH_ACCESS_TOKEN, accessToken)
        : sessionStorage.removeItem(GITHUB_OAUTH_ACCESS_TOKEN);
    },
    [setGitHubAccessToken]
  );

  // In addition to state, we will save the microsoftGraphAccessToken to session storage to persist across page refresh.
  const handleMicrosoftGraphAccessTokenChange = useCallback(
    accessToken => {
